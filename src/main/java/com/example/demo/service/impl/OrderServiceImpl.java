...
import com.example.demo.model.request.UpdateDetailOrder;
import com.example.demo.model.request.UpdateStatusOrderRequest;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDetailDTO userGetDetailById(long id, long userId) {
        ...
        if (order.getStatus() == ORDER_STATUS) {
            ...
            order.setStatusText("Đơn hàng đã bị hủy !");
	...
	@Override
	public Order createOrder1(CreateOrderRequest createOrderRequest, long userId) {
		 //Kiểm tra sản phẩm có tồn tại
        Optional<Product> product = productRepository.findById(createOrderRequest.getProductId());
        if (product == null) {
            throw new NotFoundException("Sản phẩm không tồn tại!");
